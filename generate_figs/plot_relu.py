...
load_dir = 'data/experiment_relu/n100_D5000_sigma1'
ps, z1_idxs, n, num_samples = parse_experiment_files(load_dir, experiment=6)
ps = np.array(ps)
ps = ps[3:]
gammas = ps / n
min_perc = 0.01
...
for z1_idx in tqdm(range(num_z1s)):
    for pi, p in enumerate(ps):
        ...
        mses[z1_idx, pi] = np.mean((data['yhat_m0'] - data['truey_m0']) ** 2)
...
if use_std_err:
    mi_stds = mi_stds / num_z1s
plt.plot(gammas, mi_means, 'o-')
...
plt.savefig('figs/experiment_relu.pdf')
plt.show()
